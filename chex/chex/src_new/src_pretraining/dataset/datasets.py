...
class ImageWithSupervisionDataset(Dataset):
    def __init__(self,
        ...
        if config.name.startswith('mimic_cxr-'):
            if pretrain:
                ...
                mode='all'
            data_df, self.load_indices, self.load_fn, loaded_size = load_mimic_cxr_datafile(config.name, mode, image_size=image_size, load_memmap=config.load_memmap, use_image_paths=use_image_paths)
        elif config.name == VINDR_CXR_DATASET_NAME:
            data_df, self.load_indices, self.load_fn, loaded_size = load_vindr_cxr_datafile(mode, image_size=image_size, load_memmap=config.load_memmap, use_image_paths=use_image_paths)
        elif config.name == 'nih_cxr':
            data_df, self.load_indices, self.load_fn = load_cxr14_dataset(mode, use_image_paths=use_image_paths)
            loaded_size = None
        else:
        ...
        if pretrain:
            if is_train:
                ...
                random_seed=42+200
            train_limit = 200000
            val1_limit =  int(30000/8//168)*168
            val2_limit = int(30000/8//168)*168
            # train_limit = 1300
            # val1_limit =  int(3000/8//168)*168
            # val2_limit = int(3000/8//168)*168
        else:
            raise ValueError(f'pretrain is False')
            train_limit = 6000
            val1_limit = 3*98
            val2_limit = 8*98
        ...
        if config.limit_samples is None:
            if is_train:
                if len(self.sample_ids) > 18000:
                    limit = train_limit
                else:
                    limit = len(self.sample_ids) // 10
                print(f'train limit: {train_limit}')
                print(config.name)
            else:
                if config.name=="mimic_cxr-frontal-report-cig_anatboxes-cig_anatlabels-cig_anatphrases":
                    limit = min(len(self.sample_ids), val1_limit)
                else:
                    limit = min(len(self.sample_ids), val2_limit)
                print(f'val limit: {limit} datasize')
                print(config.name)
        # else:
        ...
        if config.has_class_bboxes: #mimic_cxr RE
            ...
            # ipdb.set_trace()
            self.class_bboxes: List[np.ndarray] = load_class_bboxes(data_df, config, crops=crops, rescales=rescales, column_name='clsbbox')  # sample【'x'】？ box array
        ...
        self.processor=AutoImageProcessor.from_pretrained(rad_dino_path)
    ...
    def __getitem__(self, index) -> dict:
        ...
        try:
            img_path_or_img = load_image(self.load_indices, self.load_fn, index)  #== mmap_file[self.load_indices【index】] e.g.  mmap_file[6000]  array   loadfn：partial(load_from_memmap, mmap_file=mmap_file)
                                         #idx->i                #mmap[i]->x  #idx
            # img = None
            ...
            # ipdb.set_trace()
            if isinstance(img_path_or_img, Image.Image):
                img_path_or_img = np.array(img_path_or_img, dtype=np.float32) / 255.
            ...
            if self.dataset_info.has_anatomy_bboxes:
                ...
                img_path_or_img, anatomy_boxes, anatomy_present_masks = self._transform_with_anat_boxes(img_path_or_img, anatomy_boxes, anatomy_present_masks)
                ...
                # 对于Rad-DINO始终使用路径
                # 检查dataset_info中是否存在use_image_paths属性，如果不存在则发出警告

                img_path_or_img, class_boxes = self._transform_with_class_boxes(img_path_or_img, self.class_bboxes[index])

                
                sample['target_cls_boxes'] = class_boxes
                sample['has_class_bboxes'] = self.has_class_bboxes
            else:
                img_path_or_img = self._transform(img_path_or_img)
                # warnings.warn(f'the data does not have class bboxes and anatomy bboxes')
                # warnings.warn('--------------------------------')
            sample['x'] = img_path_or_img  #图片在这里
    ...
    def _transform(self, img_path_or_img):
        if isinstance(img_path_or_img, np.ndarray):
            img = img_path_or_img
            return self.transform(image=img, bboxes=[], labels=[])['image']
        elif isinstance(img_path_or_img, str):
            ...
            raise ValueError(f'img_path_or_img must be an Image.Image or a string, but got {type(img_path_or_img)}')
    def _transform_with_anat_boxes(self, img_path_or_img, anat_bboxes, anat_present_masks):
        ...
        if isinstance(img_path_or_img, np.ndarray):
            ...
            return image, anat_bboxes, anat_present_masks#(29, all True
        else:
            raise ValueError(f'img_path_or_img must be an Image.Image or a string, but got {type(img_path_or_img)}')
    ...
    def transform_boxes(original_boxes, original_image_size, target_size=518):
        ...
        for i in range(original_boxes.shape[0]):
            ...
            transformed_box = ImageWithSupervisionDataset.transform_box(box, original_image_size, target_size)
            transformed_boxes.append(transformed_box)
            
        return np.array(transformed_boxes)

    def _transform_with_class_boxes(self, img_path_or_img, class_bboxes):
        """
        ...
        if isinstance(img_path_or_img, str):
            ...
            img_out=self.processor(images=img, return_tensors="pt").pixel_values.squeeze(0)
            # import ipdb;ipdb.set_trace()
            use_transform_fn = False
        else:
            # 如果输入已经是图像，则直接使用
            img = img_path_or_img
            use_transform_fn = True
        # 获取边界框信息
        M = class_bboxes.shape[0]
        bbox_labels = class_bboxes[:, 4].astype(np.int64)
        class_bboxes_coords = class_bboxes[:, :4]
        
        # 根据选择的转换方法处理边界框
        if use_transform_fn:
            # 使用self.transform方法(原始第一个实现)
            transformed = self.transform(image=img, bboxes=class_bboxes_coords, labels=bbox_labels)
            transformed_img = transformed['image']
            bbox_labels = np.array(transformed['labels']).astype(np.int64)
            transformed_class_bboxes = np.array(transformed['bboxes'])
        else:
            # 使用自定义的transform_boxes方法(原始第二个实现)
            transformed_img = img_out
            transformed_class_bboxes = ImageWithSupervisionDataset.transform_boxes(class_bboxes_coords, img.size)
        
        # 合并坐标和标签
        M = transformed_class_bboxes.shape[0]
        transformed_class_bboxes = np.concatenate([transformed_class_bboxes, bbox_labels[:, None]], axis=1) \
                                  if M > 0 else np.zeros((0, 5), dtype=np.float32)
        
        # 根据输入类型决定返回图像还是路径

        return transformed_img, transformed_class_bboxes
...
class MultiDataset(Dataset):
    def __init__(self, datasets: List[ImageWithSupervisionDataset]) -> None:
        ...
        if any(auto_balance):
            ...
            auto_oversampling_factors = [factor if automatic else 1 for factor, automatic in zip(auto_oversampling_factors, auto_balance)] # 数据增加到max_size
...
def build_dataloader(configs: List[DatasetConfig],
                     ...
                     is_train: bool = True,
                     use_image_paths: bool = False,
                     pretrain: bool = False) -> DataLoader:
    ...
    def collate_fn(batch: List[dict]): #多个字典变成一个字典
        collated_batch = {}
        ...
        if dataset_info.has_class_box_sentences: #RE
    ...
    if torch.distributed.is_initialized():
        ...
        return DataLoader(
            ...
            shuffle=False,    # Shuffle must be False when using sampler
            drop_last=is_train,
            pin_memory=True,
...
def load_anatomy_bboxes(data_df, config: DatasetConfig, crops, rescales, column_prefix) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for anat_name in tqdm.tqdm(config.anatomy_names,dynamic_ncols=True, leave=False):
...
def load_image(indices: Sequence, load_fn: Callable, idx: int):
    return load_fn(indices[idx])
