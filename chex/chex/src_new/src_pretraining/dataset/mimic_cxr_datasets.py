...
def load_mimic_cxr_datafile(
    dataset_name: str, split: str, image_size, load_memmap: bool = True, load_in_memory: bool = False,use_image_paths: bool = False
    ...
    log.info(f'阶段1: test train all:!<{split}>!开始加载 MIMIC CXR 数据集 {dataset_name} !<{split}>! - 图片大小 {img_size_mode}...')
    dataset_name = prepare_mimic_cxr_datasets(dataset_name)
    if load_memmap or load_in_memory:
        # log.info(f'阶段2: 开始从 memmap 下采样并加载 MIMIC CXR 图像...')
        mmap_file, file_mapping = downsample_and_load_mimic_cxr_images(img_size_mode,use_image_paths)
        if load_in_memory:
            log.info('阶段2.1: 将图像加载到内存中...')
            mmap_file = np.array(mmap_file)
    # log.info(f'阶段3: 开始读取 CSV 数据文件...')
    ...
    if load_memmap:
        # log.info('阶段4: 从 memmap 加载图像...')
        indices = [file_mapping[sample_id] for sample_id in sample_ids] # 从映射字典中获取样本ID对应的索引
        load_fn = partial(load_from_memmap, mmap_file=mmap_file)
    else:
        # log.info('阶段4: 从 jpg 文件加载图像...')
        ImageFile.LOAD_TRUNCATED_IMAGES = True
...
def prepare_mimic_cxr_datasets(
    ...
    # log.info(f'准备阶段2: 检查处理目录 {MIMIC_CXR_PROCESSED_DIR}')
    ...
    if os.path.exists(os.path.join(MIMIC_CXR_PROCESSED_DIR, f'{dataset_name}.all.csv')):
        # log.info(f'准备阶段3: 数据集 {dataset_name} 已存在，跳过准备')
        return dataset_name
    log.info(f'准备阶段3: 开始准备数据集 {dataset_name}...')
    ...
    # MIMIC CXR
    log.info(f'准备阶段4: 使用 MIMIC-CXR-JPG 文件夹: {MIMIC_CXR_JPG_DIR}')
    ...
    if any(tag in MIMIC_CXR_REPORT_TAGS for tag in dataset_tags):
        log.info(f'准备阶段5: 使用 MIMIC-CXR-REPORTS 文件夹: {MIMIC_CXR_REPORTS_DIR}')
    ...
    if any(tag in CHEST_IMAGENOME_TAGS for tag in dataset_tags):
        log.info(f'准备阶段6: 使用 CHEST IMAGENOME 文件夹: {CHEST_IMAGEGENOME_DIR}')
    ...
    if any(tag in MSCXR_TAGS for tag in dataset_tags):
        log.info(f'准备阶段7: 使用 MS CXR 文件夹: {MS_CXR_DIR}')
...
def prepare_chest_imagenome(
    ...
    if not os.path.exists(anat_bboxes_file) \
        ...
        for sg_path in tqdm(scene_graphs, dynamic_ncols=True, leave=False):
...
def prepare_mimic_cxr_reports(
    ...
    if not os.path.exists(processed_reports_file):
        ...
        for report_path in tqdm(reports, dynamic_ncols=True, leave=False):
...
def downsample_and_load_mimic_cxr_images(size_mode: int,use_image_paths: bool = False) -> Tuple[np.ndarray, Dict[str, int]]:
    downsampled_info_path = os.path.join(MIMIC_CXR_PROCESSED_DIR, f'downsampled_{size_mode}_frontal_mapping.csv') #已经存在
    if use_image_paths:
        downsampled_path = os.path.join(MIMIC_CXR_PROCESSED_DIR, f'downsampled_{size_mode}_frontal_mapping_paths.csv')
    else:
    ...
    if os.path.exists(downsampled_path):
        if not use_image_paths: #使用全部的数据
            log.info(f'使用<图像>的memmap数据 {downsampled_path}')
            ...
            log.info(f'下采样阶段2: 加载 memmap 文件，共 {n_rows} 行')
            mmap_file = np.memmap(downsampled_path, mode='r', dtype='float32', shape=(n_rows, size_mode, size_mode))
            return mmap_file, file_mapping  #返回memmap文件和映射字典
        else:
            # log.info(f'使用<路径>csv的数据 {downsampled_path}')
            file_mapping = pd.read_csv(downsampled_info_path, usecols=['sample_id', 'index'], index_col='sample_id')
            file_mapping: Dict[str, int] = file_mapping.to_dict(orient='dict')['index']
            n_rows = len(file_mapping)
            log.info(f' 加载 路径 文件，共 {n_rows} 行')
            
            # 加载路径映射
            path_mapping = pd.read_csv(os.path.join(MIMIC_CXR_PROCESSED_DIR, f'downsampled_{size_mode}_frontal_mapping_paths.csv'), 
                                      usecols=['index', 'image_path'])
            path_mapping = path_mapping.set_index('index')['image_path'].to_dict()
            
            return path_mapping, file_mapping  #返回路径映射字典和映射字典

    assert os.path.exists(MIMIC_CXR_JPG_DIR)
    log.info(f'下采样阶段1: 开始将图像下采样到 {size_mode} (保存到 {downsampled_path})...')
    ImageFile.LOAD_TRUNCATED_IMAGES = True
    img_dir = os.path.join(MIMIC_CXR_JPG_DIR, 'files')
    log.info(f'下采样阶段2: 读取元数据...')
    mimic_cxr_meta = pd.read_csv(
    ...
    # log.info(f'下采样阶段3: 开始处理 {len(mimic_cxr_meta)} 张图像...')
    if not os.path.exists(downsampled_info_path):
        file_mapping = []
    n_rows = mimic_cxr_meta.shape[0]
    if not use_image_paths:
        ...
        for i, (row_index, row) in tqdm(enumerate(mimic_cxr_meta.iterrows()), total=n_rows, dynamic_ncols=True, leave=False):
            if not os.path.exists(downsampled_info_path):
            ...
            mmap_file[i, :, :] = img
            mmap_file.flush() # 刷新内存映射文件到磁盘
        if not os.path.exists(downsampled_info_path): #如果映射文件不存在，则保存映射文件
    ...
    if use_image_paths:
        ...
        for i, (row_index, row) in tqdm(enumerate(mimic_cxr_meta.iterrows()), total=n_rows, dynamic_ncols=True, leave=False):
            if not os.path.exists(downsampled_info_path):
                file_mapping.append((row['sample_id'], i))
            path_mapping.append((i, row['image_path']))
        ...
        if not os.path.exists(downsampled_info_path): #如果映射文件不存在，则保存映射文件
            ...
            file_mapping: Dict[str, int] = file_mapping.to_dict(orient='dict')['index']
        # 保存路径映射
        pd.DataFrame(path_mapping, columns=['index', 'image_path']).to_csv(
            os.path.join(MIMIC_CXR_PROCESSED_DIR, f'downsampled_{size_mode}_frontal_mapping_paths.csv'))
        
        # 返回路径映射字典
        
        path_mapping_dict = {idx: path for idx, path in path_mapping}
        if isinstance(file_mapping, dict):
            return path_mapping_dict, file_mapping
        else:
            return path_mapping_dict, {key: value for key, value in file_mapping}
...
def exist_mimic_cxr_jpg_images(mimic_cxr_jpg_path) -> bool:
    ...
    for i, row in tqdm(pd.read_csv(meta_data_file).iterrows(), dynamic_ncols=True, leave=False):
        p = 'p' + row['subject_id']
        p_short = p[:3]
        s = 's' + row['study_id']
...
def match_sentences(mimic_cxr_meta_df, sentence_col, anat_phrase_col, 
    ...
    for row in tqdm(df.to_dict(orient='records'), dynamic_ncols=True, leave=False):
...
def run_prepare_mimic_cxr_datasets(dataset_name, physionet_user, physionet_pw, mimic_cxr_dir, size_mode):
    ...
    downsample_and_load_mimic_cxr_images(size_mode,use_image_paths=True)
