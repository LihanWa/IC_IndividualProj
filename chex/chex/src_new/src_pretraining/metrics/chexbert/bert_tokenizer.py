...
def tokenize(impressions, tokenizer):
    ...
    for i in tqdm(range(impressions.shape[0])):
        try:
            ...
            #txt_file_name = os.path.join("~/region-guided-chest-x-ray-report-generation/src/", "failed_tokenizations.txt")
            #with open(txt_file_name, "a") as f:
            #    f.write(f"Failed tokenization for {impressions.iloc[i]} at index {i}\n")
            log.warning(f"Failed tokenization for {impressions.iloc[i]} at index {i}")
            tokenized_imp = None
