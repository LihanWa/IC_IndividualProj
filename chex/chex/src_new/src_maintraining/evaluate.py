...
#     model, checkpoint_dict = load_model_by_name( #在load模型 Chex 
#         config.model_name,
...
#                                 #训练参数:model,para,act,scale  任务:task prompt sentence     test                      False
    
#     #用yield一步步载入不同数据
#     # config.bootstrap=False#修改
#     # config.optimize_inference=False#修改 可以提高评估结果的统计可靠性，但会增加计算开销。
#     for task_name, task_config, task_dl, task_dl_val in x: #此时第 一个buildloader已经形成 sanmple 在 task_dl
#         # ipdb.set_trace()
#         #task_dl 包含数据
