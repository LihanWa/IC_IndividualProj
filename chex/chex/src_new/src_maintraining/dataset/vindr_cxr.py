...
def load_vindr_cxr_datafile(split: str, image_size, load_memmap: bool = True, load_in_memory: bool = False,use_image_paths: bool = False
    ...
    if load_memmap or load_in_memory:
        ...
        mmap_file, file_mapping = downsample_and_load_vindr_cxr_images(img_size_mode,use_image_paths)
        if load_in_memory and not use_image_paths:
...
def downsample_and_load_vindr_cxr_images(size_mode: int,use_image_paths: bool = False) -> Tuple[np.ndarray, Dict[str, int]]:
    ...
    if use_image_paths:
        downsampled_path = os.path.join(VINDR_CXR_PROCESSED_DIR, f'downsampled_{size_mode}_frontal_mapping_paths.csv')
    else:
    ...
    if os.path.exists(downsampled_path):
        if not use_image_paths:
            ...
            file_mapping: Dict[str,int] =file_mapping.to_dict(orient='dict')['index']
            n_rows = len(file_mapping)
            path_mmaping =pd.read_csv(downsampled_path,usecols=['index','image_path'],index_col='index')['image_path'].to_dict()
            return path_mmaping,file_mapping
    ...
    if use_image_paths:
        ...
        for i, image_id in tqdm(enumerate(image_ids)):
            if not os.path.exists(downsampled_info_path):
                file_mapping.append((image_id, i))
            path=os.path.join(os.environ.get('CHEX_DIR'),f"chex/chex/dataset/vindr-cxr/processed/images_png/{image_id}.png")
            path_mapping.append((i, path))
...
def run_prepare_vindr_cxr_dataset(val_ratio, size_mode):
    ...
    downsample_and_load_vindr_cxr_images(size_mode,use_image_paths=True)
