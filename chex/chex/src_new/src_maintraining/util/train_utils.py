...
#                              best_results, config, step, saved_components=(), is_best=False):
    ...
    if step > 0:
        for chkpt_path in glob.glob(os.path.join('checkpoints', f'checkpoint_*.pth')):
            # if not chkpt_path.endswith(f'checkpoint_{step:09d}.pth') and not chkpt_path.endswith('checkpoint_best.pth') and not chkpt_path.endswith('checkpoint_000000400.pth'):
