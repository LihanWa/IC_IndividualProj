...
def plot_grounding(model_output: 'ChEX', max_samples: int = 10):
    ...
    for i in range(max_samples):
        ...
        # 统一处理所有类型的图像对象
        if isinstance(fig, (plt.Figure, Image.Image, np.ndarray)):
            # 转换为numpy数组
            if isinstance(fig, plt.Figure):
                ...
                img_arr = np.array(fig)
            else:  # np.ndarray
                img_arr = fig
                
            # 确保图像是正确的格式
            if len(img_arr.shape) == 2:  # 如果是灰度图
                img_arr = np.stack([img_arr] * 3, axis=-1)
            elif len(img_arr.shape) == 3 and img_arr.shape[2] == 4:  # 如果是RGBA
                img_arr = img_arr[:, :, :3]
                
            # 直接返回numpy数组而不是wandb.Image对象，避免JSON序列化错误
            figs.append(img_arr)
        else:
            raise ValueError(f"Unexpected figure type: {type(fig)}")
...
def plot_grounding_sample(img, sentences,
    ...
    # (S x S_r)
    sent_region_l2_pairwise = torch.cdist(sentence_features.float(), box_features.float(), p=2)
    # (S)
    sentence_region_l2 = sent_region_l2_pairwise.diagonal()
    # (S x S_r)
    sent_region_cos_pairwise = torch.nn.functional.cosine_similarity(sentence_features.unsqueeze(1).float(), box_features.unsqueeze(0).float(), dim=-1)
    # (S)
    ...
    *shape_patch, d = patch_features.shape
    # (S x H*W)
    sentence_patch_cos_pairwise = torch.nn.functional.cosine_similarity(sentence_features.unsqueeze(1).float(), patch_features.view(-1, d).unsqueeze(0).float(), dim=-1)
    # (H*W)
    ...
    if n_sentences <= 10:
        sentence_colors = list(mcolors.TABLEAU_COLORS.values())[:n_sentences]
    else:
        sentence_colors = plt.cm.tab20(np.linspace(0, 1, 20))[:n_sentences]
    sentences = [twp.fill(s, 70) for s in sentences]

    # 修复颜色映射问题：确保至少有两种颜色
    if n_sentences == 1:
        # 当只有一个句子时，添加第二个颜色（可以是相同颜色的变体）
        cmap = mcolors.LinearSegmentedColormap.from_list("cmap_name", 
                                                        [sentence_colors[0], 
                                                         sentence_colors[0]], 
                                                        N=2)
    else:
    ...
    # 检查图像维度，如果是彩色图像(3通道)，正常显示；如果是灰度图像，使用cmap='gray'
    if len(img.shape) == 3 and img.shape[2] == 3:
        ax_patch_neighbors.imshow(img)
    else:
        # 检查图像形状，处理通道顺序问题
        ...
        ax_patch_neighbors.imshow(img, cmap='gray' if len(img.shape) == 2 else None)

    # 修复：使用norm参数来正确映射颜色，而不是依赖于数据范围
    norm = mcolors.Normalize(vmin=1, vmax=max(n_sentences, 2))
    ax_patch_neighbors.imshow(patch_sentence_neighbor_upsampled, cmap=cmap, 
                             norm=norm, alpha=0.6)
