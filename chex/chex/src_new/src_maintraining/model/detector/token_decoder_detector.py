...
class TokenDetector(BaseModel):
    ...
    def forward(self, 
        ...
        # ipdb.set_trace()
        box_params = self.predict_boxes(query_features) #box prediction
    ...
    def apply_roi_pool(self, 
        ...
        patch_features = self.patch_projector(encoded_image.patch_features) # (N x H x W x d)
        box_features, query_patch_map = self.roi_pool(patch_features, box_params, beta=beta)
        # (N x Q x d) 
        if skip_roi_pool:#如果给text prompt
