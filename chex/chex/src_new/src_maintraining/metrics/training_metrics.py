...
class TrainingMetrics(nn.Module):
    ...
    def update(self, model_output: "ImgTextTrainOutput"):
        ...
        if model_output.encoded_sentences is not None:
            ...
            # (N x S x S_r)
            cos_pairwise = F.normalize(sent_features.float(), dim=-1) @ F.normalize(reg_features.float(), dim=-1).transpose(1, 2)
            # (N x S)
    ...
    def compute(self):
        metrics = {
            ...
            "recon/rank": (self.mean_rank / self.count).cpu(),  #对应多个数（N）
            **{k: v.cpu() for k, v in self.box_stats.compute().items()},
            **{k: v.cpu() for k, v in self.step_metrics_meter.compute().items()}, #只对应一个数字， 将v转到cpu
        }
