...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, text_encoder):
        ...
        self.token_embed = text_encoder.token_embedding
        ...
        ckpt_dict = torch.load('/rds/general/user/lw1824/home/MCPL/MCPL/D:/124/MGCA-main/model/ckpts/MGCA/2025_05_15_11_19_48/epoch=8-step=11448.ckpt', map_location=torch.device('cpu'))
        # 从检查点提取prompt_learner相关参数
        prompt_learner_params = {k.replace('prompt_learner.', ''): v 
                                for k, v in ckpt_dict['state_dict'].items() 
                                if 'prompt_learner' in k}
        
        # 加载模型参数
        if prompt_learner_params:
            missing, unexpected = self.load_state_dict(prompt_learner_params, strict=False)
            print(f"加载了 {len(prompt_learner_params)} 个prompt_learner参数")
            print(f"缺失的参数: {missing}")
            print(f"意外的参数: {unexpected}")
        # 冻结所有参数
        for param in self.parameters():
            param.requires_grad = False
    def forward(self,JP_sents, BL_sents):
        ctx_img = self.ctx_img

        flattened_sents_JP, sents_JP_mask = flatten_prompts(JP_sents, device='cuda')
        flattened_sents_BL, sents_BL_mask = flatten_prompts(BL_sents, device='cuda')
        
        tokenized_token_JP = clip.tokenize(flattened_sents_JP, context_length=77)
        tokenized_token_BL = clip.tokenize(flattened_sents_BL, context_length=77)
        texts_JP = self.token_embed(tokenized_token_JP).type(self.dtype)  #token embedding 
        texts_BL = self.token_embed(tokenized_token_BL).type(self.dtype)
        ctx_text = self.ctx_text #4* 可学习向量
        # ipdb.set_trace()
        if ctx_text.dim() == 2:
            ctx_text = ctx_text.unsqueeze(0).expand(len(tokenized_token_JP), -1, -1)
        if JP_sents is None: ipdb.set_trace()
        # imgs_JP = self.proj_JB(self.token_embed(clip.tokenize([' '.join(JP_sents)], context_length=77)).type(self.dtype))
        imgs_JP = self.proj_JB(self.token_embed(clip.tokenize([' '.join(JP_sents[i]) for i in range(len(JP_sents))], context_length=77)).type(self.dtype))
        # 复制MAPLE_LENGTH2个序列
        # imgs_JP = imgs_JP.repeat(1, MAPLE_LENGTH2, 1)
        imgs_BL = self.proj_JB(self.token_embed(clip.tokenize([' '.join(BL_sents[i]) for i in range(len(BL_sents))], context_length=77)).type(self.dtype))
        # imgs_BL = self.proj_JB(self.token_embed(clip.tokenize([' '.join(BL_sents)], context_length=77)).type(self.dtype))
        ...
        return ctx_text, texts_JP, texts_BL, imgs_JP, imgs_BL, ctx_img, self.JP_prompts_text, self.BL_prompts_text, textual_prompts, visual_prompts
...
class ChEX(BaseModel):
    ...
    def __init__(self, config: ChEXConfig, from_checkpoint=False) -> None:
        ...
        self.prompt_learner = MultiModalPromptLearner(self.txt_encoder)
    ...
    def forward(self, 
        ...
        # tokenized_texts=caps_77
        # sentences_tokens=caps
        ctx_text, texts_JP, texts_BL, imgs_JP, imgs_BL, shared_ctx, JP_prompts_text, BL_prompts_text, deep_prompts_text, deep_prompts_vision = self.prompt_learner(JP_sents, BL_sents)
        # ipdb.set_trace()
        # encoded_image: ImageEncoderOutput = self.img_encoder(x, epoch=epoch,sample_id=sample_id)
        # ipdb.set_trace()
        use_prompt_learning=True
        # ipdb.set_trace()
        encoded_image: ImageEncoderOutput = self.img_encoder(x, use_prompt_learning,shared_ctx=shared_ctx, JP_prompts=JP_prompts_text, BL_prompts=BL_prompts_text, compound_deeper_prompts=deep_prompts_vision, imgs_JP=imgs_JP, imgs_BL=imgs_BL)
        # ipdb.set_trace()
        ...
        if self.requires_sentence_tokens and not no_sent_token_supervision:
            ...
            if not sentences or all(sent is None or len(sent) == 0 for sent in sentences):
                ...
                encoded_sentences: TextEncoderOutput = self.encode_sentences(sentences,ctx_text, JP_prompts_text, BL_prompts_text, deep_prompts_text, texts_JP, texts_BL, device=encoded_image.device, epoch=epoch)
        else:
            encoded_sentences = None
    ...
    def encode_sentences(self, sentences: List[List[str]], ctx_text, JP_prompts_text, BL_prompts_text, deep_prompts_text, texts_JP, texts_BL,device, epoch=None) -> TextEncoderOutput:
        # ipdb.set_trace()
        flattened_sentences, sentence_mask = flatten_prompts(sentences, device=device)

        # ipdb.set_trace()
        flat_sentence_features = self.txt_encoder.encode_sentences(flattened_sentences, ctx_text,JP_prompts_text, BL_prompts_text, deep_prompts_text, texts_JP, texts_BL, cache=self.config.cache_encoded_sentences)
    ...
    def encode_prompts(self, prompts_by_class: Dict[str, List[str]], class_names: List[str], region_templates: Optional[Dict[str, List[str]]]=None, epoch=None, random_synonym: bool = False):
        ...
        # ipdb.set_trace()
        flat_prompt_embeddings = self.txt_encoder.encode_sentences(flattened_prompts,  cache=True)
    ...
    def detect_prompts(self,  # img->post decoder
        ...
        box_prompts_emb: FloatTensor, 
        box_prompt_mask: Optional[FloatTensor] = None, 
        clip_boxes: bool = False, 
        skip_roi_pool: Optional[bool] = None, 
        use_post_decoder: bool = True, 
        use_prompt_learning: bool = False,
        JP_sents: List[List[str]] = None,
        BL_sents: List[List[str]] = None,
        **kwargs  # 保留kwargs以支持其他可能的参数
    ) -> TokenDetectorOutput:
        """
    ...
    def encode_regions(self, #图像、box -> postdecoder output
        x: FloatTensor, 
