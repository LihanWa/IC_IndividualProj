...
class SentenceMetrics(nn.Module):
    def __init__(self, 
        ...
        self.use_ce = use_ce
        if use_ce:
            self.ce_metrics = CEMetrics(per_class=ce_per_class, per_region=per_region, sample_macro=True, micro=True, macro=True) #后半部分结果
...
def _get_chexbert(): 
    try:
        ...
        # 清理状态字典中可能的钩子相关项
        keys_to_remove = [k for k in state_dict.keys() if 'hook' in k]
