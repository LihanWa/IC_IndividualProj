...
def load_clip_to_cpu():
    ...
    model = model.eval() 
    for p in model.parameters(): # Freeze CLIP parameters
...
class text_encoder(nn.Module):
    def __init__(self, clip_model, model):
        ...
        self.transformer = model.text_encoder.transformer           # Read transformer model
        self.positional_embedding = clip_model.positional_embedding # Positional encoding
        self.ln_final = clip_model.ln_final                         # layer norm
        self.text_projection = clip_model.text_projection           # Projection layer
    ...
    def forward(self, texts, tokenized_prompts, JP_prompts, BL_prompts, compound_prompts_text, texts_JP, texts_BL):
        ...
        x = texts + self.positional_embedding.type(self.dtype)      # Text + positional encoding
        x = x.permute(1, 0, 2)                                      # NLD -> LND
        x1 = texts_JP + self.positional_embedding.type(self.dtype)
        x2 = texts_BL + self.positional_embedding.type(self.dtype)
        x1 = x1.permute(1, 0, 2)
        x2 = x2.permute(1, 0, 2)
        combined = [x, compound_prompts_text, 0, x1, x2, JP_prompts, BL_prompts, torch.empty(x.shape[1], 6, 154)]     # Text and prompts form input
        outputs = self.transformer(combined)
        x = outputs[0]                                              # Extract first output vector
        x = x.permute(1, 0, 2)                                      # LND -> NLD
        x = self.ln_final(x).type(self.dtype)                       # batchsize tokensize dim 8 77 512
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection # Select max value for each row of x 8 512
        return x
...
class mm_prompter(nn.Module):
    ...
    def forward(self, batch):
        ...
        prefix = texts[:, :1, :]
        suffix = texts[:, 5:77, :]
        texts = torch.cat([prefix, ctx_text, suffix], dim=1) # Insert prompts into text features
        ...
        ''''''
        # imgs_JP = self.proj_JB(texts_JP) # Convert text to image features
        # imgs_BL = self.proj_JB(texts_BL)
        
        imgs_JP = self.proj_JB(embed_JP) # Convert text to image features
        imgs_BL = self.proj_JB(embed_BL)
        
        # Text prompts and visual prompts interaction
