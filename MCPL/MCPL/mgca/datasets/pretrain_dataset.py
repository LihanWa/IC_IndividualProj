...
import re
import logging
import regex as re  # 替换 import re
...
class SimpleTokenizer:
    def __init__(self):
        ...
        # 这里需要加载BPE词汇表，但为了简化，我们先使用一个基本的词汇表
        self.encoder = {
            '<|startoftext|>': 0,
            '<|endoftext|>': 1,
            # 添加其他基本token
    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            ...
            tokens.append(self.encoder.get(token, self.encoder['<|endoftext|>']))
        return tokens
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77) -> torch.LongTensor:
    ...
    -------
    torch.LongTensor
        形状为[文本数量, context_length]的token ID张量
    """
    ...
    for i, tokens in enumerate(all_tokens):
        if len(tokens) > context_length:
            ...
            log.warning(f"Input {texts[i]} is too long for context length {context_length}")
...
class MultimodalPretrainingDataset(data.Dataset):
    ...
    def load_text_data(self, split):
        ...
        if not os.path.isfile(filepath):
            ...
            with open(filepath, "rb") as f:
                path2sent = pickle.load(f) #{jpg:sent }
        ...
        for row in self.df.itertuples():
            ...
            # path=path.replace('/rds/general/user/lw1824/home/chex/chex/dataset/MIMIC-CXR/','')
            # ipdb.set_trace()
            ''''''
            idx = []
            for i in range(14):
    ...
    def __len__(self):
        ...
        # 修改后：只使用部分数据
        max_files = int(len(self.filenames)/2)  # 例如只使用1000个文件或更少
        print(f'使用 {max_files}/{len(self.filenames)} 个文件进行训练')
        return max_files
    ...
    def get_caption(self, path):
        ...
        # 按列表过滤
        JP_expect = [s1 for s1 in JP_list if s1 in sent_token] #['pleural', 'pulmonary', 'silhouette', 'mediastinal', 'interstitial']
        BL_expect = [s1 for s1 in BL_list if s1 in sent_token]
        
        JP_idx = [pos for pos, char in enumerate(JP_list) if char in JP_expect]
        if JP_idx == []: #[0, 2, 5, 6, 20]
            JP_idx = [75]
        ...
        JP_tokens = clip.tokenize(JP_sent, truncate=True) #'pleural pulmonary silhouette mediastinal interstitial'
