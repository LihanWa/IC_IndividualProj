...
reports_path = os.path.join(MIMIC_CXR_DATA_DIR, "files")
output_path = os.path.join(MIMIC_CXR_DATA_DIR, "mimic_cxr_sectioned.csv")
no_split = True
...
def extract_mimic_text():
    ...
    for p_grp in p_grp_folders:
        ...
        cxr_path = os.path.join(reports_path, p_grp)
        ...
        for p in tqdm(p_folders):
            patient_path = os.path.join(cxr_path, p)
            ...
            for s in studies:
                ...
                with open(os.path.join(patient_path, s), 'r') as fp:
    ...
    if len(patient_studies) > 0:
        ...
        with open(os.path.join(output_path, 'mimic_cxr_sectioned.csv'), 'w') as fp:
        ...
        if no_split:
            ...
            with open(os.path.join(output_path, f'mimic_cxr_sections.csv'), 'w') as fp:
                csvwriter = csv.writer(fp)
            ...
            while n < len(patient_studies):
                n_fn = n // jmp
                with open(os.path.join(output_path, f'mimic_cxr_{n_fn:02d}.csv'), 'w') as fp:
                    csvwriter = csv.writer(fp)
