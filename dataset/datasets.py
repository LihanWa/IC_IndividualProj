...
class ImageWithSupervisionDataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        try:
            img_path = load_image(self.load_indices, self.load_fn, index)  #== mmap_file[self.load_indices【index】] e.g.  mmap_file[6000]  array   loadfn：partial(load_from_memmap, mmap_file=mmap_file)
    ...
    def _transform_with_class_boxes(self, img_path, class_bboxes):
        # transform image
        img = Image.open(img_path)
        ...
        # ipdb.set_trace()
        transformed_class_bboxes = ImageWithSupervisionDataset.transform_boxes(class_bboxes, img.size)
        # print(f'original_boxes: {class_bboxes} \n transformed_boxes: {transformed_class_bboxes}')
        M = transformed_class_bboxes.shape[0]
        transformed_class_bboxes = np.concatenate([transformed_class_bboxes, bbox_labels[:, None]], axis=1) if M > 0 else np.zeros((0, 5), dtype=np.float32)

        return img_path, transformed_class_bboxes
    ...
    #     bbox_labels = class_bboxes[:, 4].astype(np.int64)
    #     # 保存原始边界框以便比较
    #     class_bboxes_original = class_bboxes[:, :4].copy()
        
    #     # 应用变换
    #     transformed = self.transform(image=img, bboxes=class_bboxes[:, :4], labels=bbox_labels)
    ...
    # # def s   
    # # 检查边界框是否发生变化
    # # 使用线程锁确保不同批次的数据不会同时执行此代码块
    # # import threading
    # # _transform_lock = threading.Lock()
    
    # # with _transform_lock:
    # #     print("边界框变换前后比较:")
    # #     print(f"原始边界框形状: {class_bboxes_original.shape}")
    # #     print(f"变换后边界框形状: {class_bboxes.shape}")
        
    # #     # 如果有边界框，比较第一个边界框的变化
    # #     if len(class_bboxes_original) > 0 and len(class_bboxes) > 0:
    # #         print(f"第一个边界框变换前: {class_bboxes_original[0]}")
    # #         print(f"第一个边界框变换后: {class_bboxes[0]}")
            
    # #         # 计算变化量
    # #         if len(class_bboxes) == len(class_bboxes_original):
    # #             diff = np.abs(class_bboxes - class_bboxes_original).mean()
    # #             print(f"边界框平均变化量: {diff:.6f}")
    # # endregion
