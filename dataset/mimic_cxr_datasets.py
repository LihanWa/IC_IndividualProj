...
#         mmap_file, file_mapping = downsample_and_load_mimic_cxr_images(img_size_mode,sample_ids)
#         if load_in_memory:
...
#     import ipdb
#     # ipdb.set_trace()
#     sample_ids = [sample_id for sample_id in sample_ids if sample_id in file_mapping] #修改
...
#         indices = [file_mapping[sample_id] for sample_id in sample_ids if sample_id in file_mapping] #修改 mapping value 短小的那个
#         load_fn = partial(load_from_memmap, mmap_file=mmap_file) #mmap_file[i]
#     else:
...
# # def downsample_and_load_mimic_cxr_images(size_mode: int,sample_ids) -> Tuple[np.ndarray, Dict[str, int]]:
# def downsample_and_load_mimic_cxr_images(size_mode, sample_ids, mimic_cxr_meta, dataset_name, save_dir="mimic_mmap"):
#     os.makedirs(save_dir,exist_ok=True)
#     mmap_file_path=os.path.join(save_dir,f'{dataset_name}_img.dat')
#     downsample_and_load_mimic_cxr_images
...
#     # n_samples = 1000  # 只处理前 1000 条数据
#     # mimic_cxr_meta = mimic_cxr_meta.iloc[:n_samples] #修改
#     import ipdb
#     # ipdb.set_trace()
#     for i, (row_index, row) in tqdm(enumerate(mimic_cxr_meta.iterrows()), total=n_rows):
#         if row['sample_id'] not in sample_ids: continue
#         try:
#             # ipdb.set_trace()
#             file_mapping.append((row['sample_id'], i))
#             img = load_pil_gray(row['image_path'])
#             img = np.array(img, dtype=np.float32) / 255.
#             img = pad_resize_transform(image=img)['image'] #变化了
...
#             # 跳过文件路径不存在的情况
#             log.warning(f"File not found: {row['image_path']}, skipping sample_id: {row['sample_id']}")
#             continue

#         except Exception as e:
#             # 捕获其他错误并记录日志
#             log.error(f"Error processing sample_id: {row['sample_id']} at index {i}, skipping. Error: {e}")
#             continue
#     import ipdb
#     # ipdb.set_trace()
#     print(f'download to .....{downsampled_info_path}')
...
def downsample_and_load_mimic_cxr_images(size_mode: int,sample_ids) -> Tuple[np.ndarray, Dict[str, int]]:
    ...
    if os.path.exists(downsampled_path):
        ...
        file_mapping: Dict[str, int] = file_mapping.to_dict(orient='dict')['index']
        n_rows = len(file_mapping)
        # mmap_file_path='/vol/bitbucket/lw1824/chex/chex/dataset/MIMIC-CXR/mimic-cxr-jpg_2-0-0/mmap_file.npy'
        # if os.path.exists(mmap_file_path):
        #     mmap_file = np.load(mmap_file_path, mmap_mode='r+')
        #     return mmap_file, file_mapping
        # mmap_file = np.memmap(downsampled_path, mode='w+', dtype='float32', shape=(n_rows, size_mode, size_mode))
        mmap_file = [None]*n_rows  # 创建长度为n_rows的列表
        pad_resize_transform = A.Compose([A.SmallestMaxSize(max_size=size_mode, interpolation=cv2.INTER_AREA), A.CenterCrop(height=size_mode, width=size_mode)])
        
        for sample_id in tqdm(sample_ids, desc="Updating mmap"):
            i = file_mapping[sample_id]  # 找到 mmap_file 需要更新的位置
            # 从 mimic_cxr_meta 里查找对应的行
            row = mimic_cxr_meta.iloc[i]
            assert row['sample_id'] == sample_id

            # if row.empty or pd.isna(row['downsampled_path'].values[0]):
            #     continue  # 没有 downsampled_path，则跳过
            # img = load_pil_gray(row['image_path'])
            # img = np.array(img, dtype=np.float32) / 255.
            # img = pad_resize_transform(image=img)['image'] 
            # assert img.shape == (size_mode, size_mode)  
            mmap_file[i] = row['image_path']
            # mmap_file.flush()
        # ipdb.set_trace()
        # np.save('/vol/bitbucket/lw1824/chex/chex/dataset/MIMIC-CXR/mimic-cxr-jpg_2-0-0/mmap_file.npy', mmap_file)
        return mmap_file, file_mapping
    else: #一般用不到，因为文件存在
        file_mapping = []
        n_rows = mimic_cxr_meta.shape[0]
