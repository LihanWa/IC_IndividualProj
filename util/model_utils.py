    ...
    def __init__(self, model_module: types.ModuleType) -> None:
        ...
        def register_model_class(model_cls): #chex model file 里面的model class
            ...
            cs.store(
                ...
                node=config_cls(model_module=module_name, model_class=model_cls_name)) # 设置config 
            #等价ChexzeroImageEncoderConfig(model_module=module_name, model_class=model_cls_name)
...
def instantiate_model(
    ...
    **kwargs) -> BaseModel: #config_dict, config_override=config_override, from_checkpoint=True
    ...
    # ipdb.set_trace()
    print('model_module: ',model_module) #'model'
...
def load_model_by_name(model_name: str, model_component: Optional[str] = None, step: Optional[int] = -1, 
    ...
    run_dir = get_run_dir(model_dir, run_name='run_0') #修改
    # ipdb.set_trace()
...
#     # # ckpt_dict = broadcast_checkpoint(checkpoint_path,device,rank)
#     # ckpt_dict=torch.load(checkpoint_path)
#     # assert 'state_dict' in ckpt_dict and 'config_dict' in ckpt_dict, f'Invalid checkpoint dict. Keys: {ckpt_dict.keys()}'
#     # config_dict = ckpt_dict['config_dict']
#     # state_dict = ckpt_dict['state_dict']
