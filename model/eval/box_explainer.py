...
class BoxExplainerEvaluator(Evaluator):
    def __init__(self, config: BoxExplainerConfig, model: 'ChEX', bootstrap=False,  results_path: Optional[str] = None, **kwargs):
        ...
        if self.config.classify_boxes: #True
            ...
            self.pos_prompt_emb, self.neg_prompt_emb = self.model.encode_pos_neg_prompts(
                ...
                #neg_prompt_mode: pos_centroid  no_finding_prompts[]
    ...
    def eval_step(self, 
        ...
        **kwargs) -> BoxExplainerOutput: #**kwargs?
        ...
        max_M = max(M_is) #target 最多有几个box
        # (N x M_max)
        ...
        # ipdb.set_trace()
        # (N x M_max x d)
        region_features, output_img = self.model.encode_regions(x, target_cls_boxes_stacked[:, :, :4], target_cls_boxe_mask)
        ...
        if self.config.classify_boxes:
            ...
            region_cls_probs, _ = self.model.classify_features(
                repeated_region_features, pos_prompt_emb, neg_prompt_emb, normalized=self.config.normalize_classification) #True
            # (N x M_max)
